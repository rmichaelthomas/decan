...
Decan does not authorize, execute, retry, verify, or fulfill work. It has no ambient locale, timezone, clock, geolocation, network, or dynamic-observer fallback. Missing context remains a typed `need`; supplied context must be explicit evidence. The checkpoint chain in [`references/`](references/) is the governing semantic record, with the explicit-context update locked in [`references/decan_checkpoint_v1_1_explicit_context_snapshots.md`](references/decan_checkpoint_v1_1_explicit_context_snapshots.md).
