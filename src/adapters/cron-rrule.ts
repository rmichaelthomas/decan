...
export type ScheduleAdapterImport = Readonly<{ source: ImportedScheduleSource; expression: CompoundExpression; lifecycle: IntentLifecycle; diagnostics: ReadonlyArray<Pick<Diagnostic, "code" | "message">>; lossReport: TemporalLossReport }>;
...
const exactImport = (kind: ImportedScheduleSource["kind"], value: string, expressionValue: CompoundExpression, effectiveFrom: DateValue, code: string, message: string): OperationResult<ScheduleAdapterImport> => ({ ok: true, value: { source: { kind, value }, expression: expressionValue, lifecycle: lifecycle(effectiveFrom), diagnostics: [{ code, message }], lossReport: exactLossReport(kind === "imported_cron" ? "cron" : "rrule", "import", ["weekly civil recurrence", "local clock point", "lifecycle origin"]) } });
...
export function importCronExpression(request: CronImportRequest): OperationResult<ScheduleAdapterImport> {
  ...
  if (dayOfMonth !== "*" || month !== "*") return unsupported("cron", "import", "DECAN-ADAPTER-CRON-UNSUPPORTED", "Only weekly cron expressions with wildcard day-of-month and month are supported.");
  if (!/^\d+$/.test(minuteText ?? "") || !/^\d+$/.test(hourText ?? "") || !/^\d+$/.test(dayOfWeekText ?? "")) return unsupported("cron", "import", "DECAN-ADAPTER-CRON-UNSUPPORTED", "Cron import requires single numeric minute, hour, and weekday fields.");
  const minute = Number(minuteText);
  const hour = Number(hourText);
  const cronDay = Number(dayOfWeekText);
  const dayOfWeek = cronDay === 0 ? 7 : cronDay;
  if (minute < 0 || minute > 59 || hour < 0 || hour > 23 || dayOfWeek < 1 || dayOfWeek > 7) return unsupported("cron", "import", "DECAN-ADAPTER-CRON-UNSUPPORTED", "Cron import fields are outside the exact supported range.");
...
export function importRRule(request: RRuleImportRequest): OperationResult<ScheduleAdapterImport> {
  ...
  if ([...rule.keys()].some((key) => !allowed.has(key))) return unsupported("rrule", "import", "DECAN-ADAPTER-RRULE-UNSUPPORTED", "RRULE contains parts outside Decan's exact weekly subset.");
  if (rule.get("FREQ") !== "WEEKLY" || (rule.get("INTERVAL") ?? "1") !== "1") return unsupported("rrule", "import", "DECAN-ADAPTER-RRULE-UNSUPPORTED", "Only weekly RRULEs with INTERVAL=1 are supported.");
  const byday = rule.get("BYDAY");
  if (byday && (!weekdayNumbers[byday] || weekdayNumbers[byday] !== plainFromDate(start.date).dayOfWeek)) return unsupported("rrule", "import", "DECAN-ADAPTER-RRULE-UNSUPPORTED", "RRULE BYDAY must be a single weekday matching DTSTART.");
...
export function exportRRule(request: RRuleExportRequest): OperationResult<RRuleExport> {
  ...
  if (!parts || !effectiveFrom) return unsupported("rrule", "export", "DECAN-ADAPTER-RRULE-UNSUPPORTED", "RRULE export supports only weekly civil recurrence with one clock point and lifecycle origin.");
  const weekday = weekdayCodes[plainFromDate(effectiveFrom).dayOfWeek];
  return { ok: true, value: { contentLines: [`DTSTART:${basicDateTime(effectiveFrom, parts.hour, parts.minute)}`, `RRULE:FREQ=WEEKLY;INTERVAL=1;BYDAY=${weekday}`], diagnostics: [{ code: "DECAN-ADAPTER-RRULE-EXACT-SUBSET", message: "Exported exact weekly RRULE subset." }], lossReport: exactLossReport("rrule", "export", ["weekly civil recurrence", "local clock point", "lifecycle origin"]) } };
}
